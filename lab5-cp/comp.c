...
struct const_assgs{
  int asgn;
  int val;
  struct const_assgs* next;
};

struct const_assgs* const_assgs_head = NULL;
struct const_assgs* last_const_asgn = NULL;
// symbol table
...
void clean_const_assgs()
{
  struct const_assgs* head = const_assgs_head;

  while(const_assgs_head != NULL)
    {
        free(head);
        const_assgs_head = const_assgs_head->next;
        head = const_assgs_head;
    }
}

void add_const_assg(int asgn, int val)
{
  if(const_assgs_head == NULL)
  {
    const_assgs_head = (struct const_assgs*)malloc(sizeof(struct const_assgs));
    const_assgs_head->asgn = asgn;
    const_assgs_head->val = val;
    const_assgs_head->next = NULL;
    last_const_asgn = const_assgs_head;
  }
  else
  {
    struct const_assgs* tmp = (struct const_assgs*)malloc(sizeof(struct const_assgs));
    last_const_asgn->next = tmp;
    tmp->asgn = asgn;
    tmp->val = val;
    tmp->next = NULL;
    last_const_asgn = tmp;  
...
int is_const_asg(int asgn)
{
  struct const_assgs* tmp = const_assgs_head;
...
void delete_extra_ops(int asgn_v)
{
  struct asgn_instr* asgn = asgn_root;
  struct asgn_instr* prev = asgn_root;
...
int cp_optimization()
{
  ...
  {
    if(asgn->type == CONST)
      add_const_assg(asgn->lhs, asgn->op1);
  ...
  {
    if (asgn->bin == 1){
      int op1 = is_const_asg(asgn->op1);
      int op2 = is_const_asg(asgn->op2);
      int val;
      int flag = 0;
      if(op1 != -999 && op2 != -999)
      {
        if (asgn->type == EQ)
        {
          delete_asgn(asgn->op1);
          delete_asgn(asgn->op2);
        ...
        else if (asgn->type == PLUS)
        {
          delete_asgn(asgn->op1);
          delete_asgn(asgn->op2);
          struct asgn_instr* last_lhs = NULL;

          val = op1 + op2;

          struct asgn_instr* tmp = asgn->next;
          //struct asgn_instr* prev = asgn;

          while(tmp != NULL && tmp->lhs == asgn->lhs)
          {
            /*if(is_const_asg(tmp->op2) == -999)
            {
              flag = 2;
              asgn = asgn->next;
              continue;
            }*/
            val += is_const_asg(tmp->op2);
            delete_asgn(tmp->op2);
            tmp = tmp->next;
            flag = 1;
            //last_lhs = tmp;
          ...
          if(flag == 1)
            delete_extra_ops(asgn->lhs);
          
          /*if(flag == 2)
          {
            //last_lhs->op1 = val;
            //asgn->op1 = last_lhs->lhs;
            asgn = asgn->next;
            continue;
          }*/
          asgn->bin = 0;
          ...
          asgn->type = CONST;
          add_const_assg(asgn->lhs, val); 
          //printf("Time no, %d asgn->lhs = %d , asgn->op1: %d asg->op2: %d asgn->type: %d, asgn->bin: %d\n", i, asgn->lhs, asgn->op1, asgn->op2, asgn->type, asgn->bin); i++;
          //printf("v%d := %d \n", asgn->lhs, asgn->op1);
        }
        else if (asgn->type == MINUS)
        {
...
int main (int argc, char **argv) {
  ...
  if(argv[1] != NULL && strcmp(argv[1], "--cp") == 0)
  {
  ...
  {
    printf("\nWithout Constant Propagation Optimization\n");
  }
