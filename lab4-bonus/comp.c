...
void print_declare_var()
{
  ...
  { 
    if (asgn->bin == 0){
      ...
      if (asgn->type == INP)
      {
        ...
        fprintf(smt_file,"(declare-var a%d ", -asgn->op1);

        //printf("(declare-var a%d ", -asgn->op1);
        if(tmp->type == BOOLID)
        {
            //printf("Bool)\n");
            fprintf(smt_file,"Bool)\n");
        }
        else
        {
            //printf("Int)\n");
            fprintf(smt_file,"Int)\n");
        }
            

        //printf("(declare-var v%d ", asgn->lhs);
        fprintf(smt_file,"(declare-var v%d ", asgn->lhs);
        if(tmp->type == BOOLID)
        {
            //printf("Bool)\n");
            fprintf(smt_file,"Bool)\n");
      ...
      else if (asgn->lhs == 0)
      {
        ...
        fprintf(smt_file,"(declare-var rv \n");
      }
  ...
  while(br != NULL)
  {
    if (br->cond > 0)
    {
      ...
      fprintf(smt_file,"(declare-var v%d ", br->cond);
...
int print_assertions()
{
  ...
  if(br != NULL)
  {
    if(br == bb_root)
    {
      ...
      fprintf(smt_file,"(assert (=> true (and\n");
    }
      ...
      if (asgn->bb != br->id){
        if (br->cond == 0){
          ...
          if (br->succ1 != -1)
          {
            ...
            if(br_parent == NULL)
            {
                //printf("(assert (=> (and (not v%d)) (and\n", last_cond);
                fprintf(smt_file,"(assert (=> true (and\n");
            }
             
            else
            {
              br_grand_parent = get_parent_br_cond(br_parent->id); 
              if(br_grand_parent == NULL || br_grand_parent->cond == 0)
              {
                if(br_parent->succ1 == br->id)
                {
                  //printf("(assert (=> (and v%d v%d) (and\n", br_parent->cond,br->cond);
                  fprintf(smt_file,"(assert (=> (and v%d v%d) (and\n", br_parent->cond,br->cond);
                }
                  
                
                if(br_parent->succ2 == br->id)
                {
                  //printf("(assert (=> (and (not v%d) v%d) (and\n", br_parent->cond,br->cond);
                  fprintf(smt_file,"(assert (=> (and (not v%d) v%d) (and\n", br_parent->cond,br->cond);
                }
                 
              }
              else
              {
                if(br_grand_parent->succ1 == br_parent->id)
                {
                  //printf("(assert (=> (and v%d v%d) (and\n", br_grand_parent->cond, br_parent->cond);
                  fprintf(smt_file,"(assert (=> (and v%d v%d) (and\n", br_grand_parent->cond, br_parent->cond);
                }
                  
                if(br_grand_parent->succ2 == br_parent->id)
                {
                  //printf("(assert (=> (and (not v%d) v%d v%d) (and\n", br_grand_parent->cond, br_parent->cond,br->cond);
                  fprintf(smt_file,"(assert (=> (and (not v%d) v%d v%d) (and\n", br_grand_parent->cond, br_parent->cond,br->cond);
                }
        ...
        else
        {
            ...
            if(br_parent == NULL)
            {
              //printf("(assert (=> (and v%d) (and\n", br->cond);
              fprintf(smt_file,"(assert (=> (and v%d) (and\n", br->cond);
            }
            else
            {
              br_grand_parent = get_parent_br_cond(br_parent->id); 
              if(br_grand_parent == NULL)
              {
                if(br_parent->succ1 == br->id)
                {
                  //printf("(assert (=> (and v%d v%d) (and\n", br_parent->cond,br->cond);
      ...
      if (asgn->bin == 0){
        if (asgn->type == CONST)
          fprintf(smt_file,"  (= v%d %d)\n", asgn->lhs, asgn->op1);
        else if (asgn->type == NOT)
          fprintf(smt_file,"  (= v%d (not v%d))\n", asgn->lhs, asgn->op1);
        else if (asgn->type == INP)
          fprintf(smt_file,"  (= v%d a%d)\n", asgn->lhs, -asgn->op1);
        else if (asgn->lhs == 0)
          fprintf(smt_file,"  (= rv v%d)\n", asgn->op1);
        else if (asgn->lhs < 0)
          fprintf(smt_file,"  (= a%d v%d)\n", -asgn->lhs, asgn->op1);
        else
          fprintf(smt_file,"  (= v%d v%d)\n", asgn->lhs, asgn->op1);

      }
      else if (asgn->bin == 1){
        if (asgn->type == EQ)
          fprintf(smt_file,"  (= v%d (= v%d v%d))\n", asgn->lhs, asgn->op1, asgn->op2);
        else if (asgn->type == LT)
          fprintf(smt_file,"  (= v%d (< v%d v%d))\n", asgn->lhs, asgn->op1, asgn->op2);
        else if (asgn->type == PLUS)
          fprintf(smt_file,"  (= v%d (+ v%d v%d))\n", asgn->lhs, asgn->op1, asgn->op2);
          ...
          fprintf(smt_file,"  (= v%d (and v%d v%d))\n", asgn->lhs, asgn->op1, asgn->op2);
        else if (asgn->type == LE)
          fprintf(smt_file,"  (= v%d (<= v%d v%d))\n", asgn->lhs, asgn->op1, asgn->op2);
        else if (asgn->type == MULT)
          fprintf(smt_file,"  (= v%d (* v%d v%d))\n", asgn->lhs, asgn->op1, asgn->op2);
        else if (asgn->type == DIV)
          fprintf(smt_file,"  (= v%d (div v%d v%d))\n", asgn->lhs, asgn->op1, asgn->op2);
        else if (asgn->type == MOD)
          fprintf(smt_file,"  (= v%d (mod v%d v%d))\n", asgn->lhs, asgn->op1, asgn->op2);
        else if (asgn->type == GT)
          fprintf(smt_file,"  (= v%d (> v%d v%d))\n", asgn->lhs, asgn->op1, asgn->op2);
        else if (asgn->type == GE)
          fprintf(smt_file,"  (= v%d (>= v%d v%d))\n", asgn->lhs, asgn->op1, asgn->op2);
        else if (asgn->type == OR)
          fprintf(smt_file,"  (= v%d (or v%d v%d))\n", asgn->lhs, asgn->op1, asgn->op2);
      }
      else if (asgn->bin == 2){
        if (strcmp(asgn->fun, "print") != 0)
          fprintf(smt_file,"  (= v%d rv)\n", asgn->lhs);
      }
      asgn = asgn->next;
    }
    fprintf(smt_file,")))\n");
