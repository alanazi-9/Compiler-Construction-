            ...
            else 
            {
              if(br_grand_parent == NULL)
              {
                ...
                if(br_parent->succ2 == br->id)
                    ...
                    printf("(assert (and v%d ", br_grand_parent->cond);
                    while(br_grand_parent != NULL)
                    {
                      while(br_parent != br_grand_parent)
                      {
                        if(br_grand_parent->succ1 == br_parent->id)
                          printf(" v%d", br_parent->cond);

                        if(br_grand_parent->succ2 == br_parent->id)
                          printf(" (not v%d)", br_parent->cond);

                        br_parent = get_parent_br_cond(br_parent->id);
                      }

                      br_grand_parent = get_parent_br_cond(br_grand_parent->id);
                    }
                    printf(")\n");
              }
