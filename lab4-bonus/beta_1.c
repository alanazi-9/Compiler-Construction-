...
void print_declare_var()
{
  ...
  { 
    if (asgn->bin == 0){
      ...
      if (asgn->type == INP)
      {
        printf("(declare-var a%d var: %s ", -asgn->op1, tmp->name);
        if(tmp->type == BOOLID || tmp->type == BOOL)
          printf("Bool)\n");
        else
          printf("Int)\n");
      }
      ...
      else if(asgn->lhs > 0 && asgn->type != NOT && asgn->type != CONST)
      {
        printf("(declare-var v%d var: %s ", asgn->lhs, tmp->name);
