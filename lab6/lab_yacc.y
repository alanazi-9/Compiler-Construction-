...
%start program
%token PLUS MINUS DIV MOD MULT EQ LT GT GE LE NOT OR AND IF LET LPAR RPAR CONST GETINT GETBOOL DEFFUN TRUE FALSE ID INT BOOL ERR PRINT EVAL CALL FUNID BOOLID INTID REGID LETID VARID INP COST
%type<val> expr funid letid intid boolid type vars exprs FUNID PLUS MINUS DIV MOD MULT EQ LT GT GE LE NOT OR AND LPAR RPAR CALL GETINT GETBOOL INT BOOL DEFFUN TRUE FALSE ERR
%type<str> ID CONST BOOLID INTID LETID VARID

%%
program : LPAR funid expr RPAR {
  insert_children (2, $2, $3);
  insert_node("ENTRY", PRINT);}
| LPAR DEFFUN LPAR funid vars RPAR type expr RPAR program {
  insert_child($4);
  for (int i = 0; i < $5; i++) insert_child(pop_int(&tmp_r, &tmp_t));
  insert_children (2, $7, $8);
...
| LPAR ID exprs RPAR {
  ...
  $$ = insert_node($2, CALL);}
| LPAR GETINT RPAR {
  $$ = insert_node("get_int", CALL);}
| LPAR GETBOOL RPAR {
  $$ = insert_node("get_bool", CALL);}
;
